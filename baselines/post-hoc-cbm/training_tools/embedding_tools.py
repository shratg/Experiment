...
def load_or_compute_projections(args, backbone, posthoc_layer, train_loader, test_loader):
    ...
    # 生成缓存文件名
    train_file = f"train-embs_{args.dataset}__{backbone_source}__{conceptbank_source}.npy"
    test_file = f"test-embs_{args.dataset}__{backbone_source}__{conceptbank_source}.npy"
    train_proj_file = f"train-proj_{args.dataset}__{backbone_source}__{conceptbank_source}.npy"
    test_proj_file = f"test-proj_{args.dataset}__{backbone_source}__{conceptbank_source}.npy"
    train_lbls_file = f"train-lbls_{args.dataset}__{backbone_source}__{conceptbank_source}_lbls.npy"
    test_lbls_file = f"test-lbls_{args.dataset}__{backbone_source}__{conceptbank_source}_lbls.npy"
