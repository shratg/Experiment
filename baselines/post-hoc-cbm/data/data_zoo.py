...
def get_dataset(args, preprocess=None):
    if args.dataset == "cifar10":
        ...
        # Keep preprocessing consistent with concept learning for CLIP backbones.
        use_clip_preprocess = preprocess is not None and "clip" in str(args.backbone_name).lower()
        train_transform = preprocess if use_clip_preprocess else None
        test_transform = preprocess if use_clip_preprocess else None
        ...
        train_loader = load_cub_data([TRAIN_PKL], use_attr=False, no_img=False,
            ...
            n_classes=num_classes, resampling=True, transform_override=train_transform)

        test_loader = load_cub_data([TEST_PKL], use_attr=False, no_img=False,
                batch_size=args.batch_size, uncertain_label=False, image_dir=CUB_DATA_DIR, resol=224, normalizer=normalizer,
                n_classes=num_classes, resampling=True, transform_override=test_transform)
        ...
        from .awa2 import load_awa2_data
        train_loader, test_loader, idx_to_class, classes = load_awa2_data(args, preprocess)
