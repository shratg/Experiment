...
def main():
    ...
    for C in concept_libs.keys():
        backbone_source = sanitize_filename(args.backbone_name)
        lib_path = os.path.join(args.out_dir, f"{args.dataset_name}_{backbone_source}_{C}_{args.n_samples}.pkl")
...
'''
# 注意：等号两边不能有空格
set OUTPUT_DIR=D:\Edge-download\post-hoc-cbm-main\post-hoc-cbm-main\conceptbanks

python learn_concepts_dataset.py --dataset-name="cub" --backbone-name="clip:RN50" --C 0.001 0.01 0.1 1.0 10.0 --n-samples=50 --out-dir="D:\Edge-download\post-hoc-cbm-main\post-hoc-cbm-main\conceptbanks"

python train_pcbm.py --concept-bank="D:\Edge-download\post-hoc-cbm-main\post-hoc-cbm-main\conceptbanks\RN50.pt" --dataset="cub" --backbone-name="resnet18_cub" --out-dir="D:\Edge-download\post-hoc-cbm-main\post-hoc-cbm-main\conceptbanks" --lam=2e-4
